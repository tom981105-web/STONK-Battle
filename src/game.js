...
const NEWS_ENABLED = false; // ★ 뉴스 기능 일시 OFF (뉴스→매수/매도 시스템 재조정 예정). true 로 바꾸면 즉시 재활성.
const NEWS_CHANCE = 0.018; // 매 틱(4초)마다 뉴스 발생 확률 → 평균 약 4분에 1번 (잦은 뉴스 충격이 차트를 난폭하게 만들어 완화)
const IPO_CHANCE = 0.008; // 진행 중 공모가 없을 때 매 틱마다 새 공모 시작 확률 (평균 약 8분에 1번)
const IPO_SUBSCRIBE_MS = 30000; // 공모주 청약 기간 (30초)

// 거래 충격(가격 영향): 실제 시장처럼 "소량은 거의 안 움직이고, 큰 주문/누적 매수세가 쌓일 때만 서서히" 반영
//   - pressure(순매수 주식 수)를 활발도(activ=유동성 대용)로 나눠 충격 산출 → 한산한 종목은 더, 활발한 종목은 덜 움직임
//   - 매 틱 0으로 리셋하지 않고 PRESSURE_DECAY 만큼 남겨, 지속 매수세는 누적되어 점진 상승(고립된 소량 주문은 몇 틱 만에 소멸)
const IMPACT_K = 0.00004;    // 순매수 1주당 기본 영향 계수 (예전 0.002 대비 ~50배 완화)
const IMPACT_CAP = 0.015;    // 틱당 거래 충격 상한 ±1.5%
const PRESSURE_DECAY = 0.55; // 매 틱 잔류 압력 비율(누적 효과). 1에 가까울수록 더 오래 끌고 감
...
// ----- 업종 구성 (전부 허구) -----
// 업종마다: 대장주 1 + 부대장주 2 + 관련주 7 + 일반주 3 + 우선주 1 = 14종목.
// 업종 10개 × 14 + 자산 10 ≈ 150종목.
const SECTORS = [
...
export function generateStocks() {
  ...
  // 가격대 다양화: 동전주(100원대) ~ 황제주(수백만원)까지 폭넓게 분포시킨다.
  const leaderPrice = () => { let p = randInt(120000, 900000); if (Math.random() < 0.4) p = Math.round(p * rand(2, 4)); return p; }; // 12만 ~ 360만
  ...
  const normalPrice = () => { const r = Math.random(); if (r < 0.3) return randInt(100, 900); if (r < 0.65) return randInt(900, 6000); return randInt(6000, 30000); }; // 동전주 포함

  // 업종별 구성: 대장주 1 + 부대장주 2 + 관련주 8 + 일반주 4 + 우선주 1 = 16종목
  SECTORS.forEach((sec) => {
    used.add(sec.leader);
...
// 거래 충격: 순매수 압력을 유동성(활발도)로 나눠 산출. 소량은 거의 0, 큰/누적은 상한까지 서서히.
function pressureImpact(s) {
  return clamp((s.pressure || 0) * IMPACT_K / (s.activ || 1), -IMPACT_CAP, IMPACT_CAP);
}
...
export async function marketTick(roomCode, roomData) {
  ...
  let newsEffect = 0;
  let newsText = "";
  if (NEWS_ENABLED && Math.random() < NEWS_CHANCE) {
    ...
    newsEffect = rand(tpl.effect[0], tpl.effect[1]) * 0.3; // 뉴스 충격 완화(하루 흐름)
  ...
  function applyStock(id, s, drift, botVolume, extra = {}) {
    ...
    // 평균회귀: 기준가에서 멀어질수록 되돌리는 약한 힘 → 한쪽으로 무한히 쓸려가 ±30% 밴드에
    // 박히는 현상 방지(실제 주식처럼 적정가 부근을 진동). 괴리가 클수록(밴드 근처) 더 강하게.
    const dev = base ? (s.price - base) / base : 0;
    drift += clamp(-dev * 0.01, -0.006, 0.006);
    ...
    // 압력 감쇠(누적): 0으로 리셋하지 않고 일부만 남겨 지속 매수세가 점진 반영되게 한다.
    if ((s.pressure || 0) !== 0) {
      const decayed = (s.pressure || 0) * PRESSURE_DECAY;
      updates[P + "pressure"] = Math.abs(decayed) < 1 ? 0 : +decayed.toFixed(2);
    }
  ...
  function equityOwn(s) {
    ...
    if (Math.random() < 0.006) heat = clamp(heat + rand(0.3, 1.0), 0, 1.6); // 가끔 테마 발동
    const effVolat = volat * (1 + heat * 0.5);
    // 추세(모멘텀): 완만한 OU 워크. 클램프를 낮춰 한 방향 폭주를 막음(실제 주식 같은 느린 흐름)
    const trend = clamp((s.trend || 0) * 0.96 + (Math.random() - 0.5) * 0.0008 * effVolat, -0.0025 * (1 + heat * 0.5), 0.0025 * (1 + heat * 0.5));
    let own = (Math.random() - 0.5) * 0.0016 * effVolat + trend;
    if (Math.random() < 0.005) own += (Math.random() - 0.5) * 0.012 * (1 + heat * 0.4); // 돌발 변동(예측 불가, 드물게)
  ...
  for (const id of ids) {
    ...
    let drift = own + pressureImpact(s) + clamp(botNet * 0.0002, -0.008, 0.008);
  ...
  for (const id of ids) {
    ...
    drift += pressureImpact(s) + clamp(botNet * 0.0002, -0.008, 0.008);
    if (id === newsTargetId) drift += newsEffect;
    const chg = applyStock(id, s, drift, botVolume, { trend, heat, news: id === newsTargetId ? newsText : null });
    moveById[id] = chg;
    equityMoves.push(chg);
  ...
  for (const id of ids) {
    ...
    drift += pressureImpact(s) + clamp(botNet * 0.0003, -0.01, 0.01);
...
export function calcRanking(players, stocks) {
  return Object.entries(players || {})
    .map(([uid, p]) => ({
      uid,
      // 닉네임이 비어 있어도 '누군지' 보이도록 폴백 (uid 끝 4자리)
      nickname: (p.nickname && String(p.nickname).trim()) || ("플레이어-" + String(uid).slice(-4)),
